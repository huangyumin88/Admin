    ...
    <div class="TUI-contact" :class="[env.isH5 ? 'TUI-contact-H5' : '']">
            ...
            <div class="search-box" v-else>
              <div class="input-box">
                <input
                  type="text"
                  v-model="searchID"
                  :placeholder="'TUIContact.输入群ID，按回车搜索'"
      ...
      >
        <header class="TUI-contact-main-h5-title" v-if="env.isH5">
          ...
          <h1>{{ currentGroup?.name || 'TUIContact.系统通知' }}</h1>
        </header>
        <div v-if="!!currentGroup?.groupID" class="TUI-contact-main-info">
          <header class="TUI-contact-main-info-header">
            <ul class="list">
              ...
              <li>
                <label>{{ 'TUIContact.群ID' }}：</label>
                <span>{{ currentGroup?.groupID }}</span>
              </li>
              <li>
                <label>{{ 'TUIContact.群类型' }}：</label>
          ...
          <main class="TUI-contact-main-info-main" v-if="isNeedPermission">
            <label>{{ 'TUIContact.请填写验证信息' }}</label>
            <textarea v-model="currentGroup.applyMessage" :disabled="currentGroup.apply"></textarea>
          </main>
          <footer class="TUI-contact-main-info-footer">
            <p v-if="currentGroup.apply && currentGroup.type === 'AVChatRoom'">{{ 'TUIContact.已加入' }}</p>
            <p v-else-if="currentGroup.apply">{{ 'TUIContact.已申请' }}</p>
            ...
            <button class="btn btn-cancel" v-else @click="quit(currentGroup)">{{ 'TUIContact.退出群聊' }}</button>
            <button
  ...
  setup(props: any, ctx: any) {
    ...
    const handleSearchGroup = async (e: any) => {
      ...
      if (data.searchID.trim()) {
        try {
          ...
          const message = 'TUIContact.该群组不存在';
    ...
    const join = async (group: any) => {
      const options: any = {
        ...
        applyMessage: group.applyMessage || 'TUIContact.加群',
        type: group?.type,
      };
