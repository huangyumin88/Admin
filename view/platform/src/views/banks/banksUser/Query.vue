...
<template>
	<ElForm class="query-form" :ref="(el: any) => { queryForm.ref = el }" :model="queryCommon.data" :inline="true" @keyup.enter="queryForm.submit">
		...
		<ElFormItem prop="id">
			<MySelect v-model="queryCommon.data.id" :placeholder="t('banks.banksUser.name.id')" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/banks//list' }" />
		</ElFormItem>
		<ElFormItem prop="user_id">
			<MySelect v-model="queryCommon.data.user_id" :placeholder="t('banks.banksUser.name.user_id')" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/user/user/list' }" />
		</ElFormItem>
		<ElFormItem prop="bank_id">
			<MySelect v-model="queryCommon.data.bank_id" :placeholder="t('banks.banksUser.name.bank_id')" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/banks/bank/list' }" />
		</ElFormItem>
