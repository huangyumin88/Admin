...
const table = reactive({
	...
	// {
	// 	dataKey: 'id',
	// 	title: t('banks.banksUser.name.id'),
	// 	key: 'id',
	// 	align: 'center',
	// 	width: 150,
	// },
	{
		dataKey: 'user_id',
		title: t('banks.banksUser.name.user_id'),
		key: 'user_id',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'bank_id',
		title: t('banks.banksUser.name.bank_id'),
		key: 'bank_id',
		align: 'center',
		width: 150,
	},
...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request(t('config.VITE_HTTP_API_PREFIX') + '/banks/banksUser/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/banks/banksUser/list', param)
	...
	<ElRow class="main-table-tool">
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
				<MyExportButton :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/banks/banksUser/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
