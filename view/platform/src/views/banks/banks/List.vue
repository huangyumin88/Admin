...
const table = reactive({
	...
	{
		dataKey: 'code',
		title: t('banks.banks.name.code'),
		key: 'code',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'flutterwaveBankCode',
		title: t('banks.banks.name.flutterwaveBankCode'),
		key: 'flutterwaveBankCode',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'redbillerBankCode',
		title: t('banks.banks.name.redbillerBankCode'),
		key: 'redbillerBankCode',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'anchorBankCode',
		title: t('banks.banks.name.anchorBankCode'),
		key: 'anchorBankCode',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'country',
		title: t('banks.banks.name.country'),
		key: 'country',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'currency',
		title: t('banks.banks.name.currency'),
		key: 'currency',
		align: 'center',
		width: 150,
	},
	...
	{
		dataKey: 'isStop',
		title: t('banks.banks.name.isStop'),
		key: 'isStop',
		align: 'center',
		width: 100,
		cellRenderer: (props: any): any => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
	request(t('config.VITE_HTTP_API_PREFIX') + '/banks/banks/info', { id: id }).then((res) => {
...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request(t('config.VITE_HTTP_API_PREFIX') + '/banks/banks/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/banks/banks/list', param)
	...
	<ElRow class="main-table-tool">
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
				<MyExportButton :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/banks/banks/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
