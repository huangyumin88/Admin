...
const table = reactive({
	...
	{
		dataKey: 'user_id',
		title: t('banks.bankCards.name.user_id'),
		key: 'user_id',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'bank_id',
		title: t('banks.bankCards.name.bank_id'),
		key: 'bank_id',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'card_number',
		title: t('banks.bankCards.name.card_number'),
		key: 'card_number',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'card_holder_name',
		title: t('banks.bankCards.name.card_holder_name'),
	...
	},
	{
		...
		title: t('banks.bankCards.name.expiration_date'),
		key: 'expiration_date',
		align: 'center',
		width: 100,
		sortable: true,
	...
	{
		dataKey: 'cvv',
		title: t('banks.bankCards.name.cvv'),
		key: 'cvv',
		align: 'center',
		width: 150,
	},
...
const handleEditCopy = (id: number, type: string = 'edit') => {
	request(t('config.VITE_HTTP_API_PREFIX') + '/banks/bankCards/info', { id: id }).then((res) => {
...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request(t('config.VITE_HTTP_API_PREFIX') + '/banks/bankCards/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/banks/bankCards/list', param)
	...
	<ElRow class="main-table-tool">
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
				<MyExportButton :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/banks/bankCards/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
