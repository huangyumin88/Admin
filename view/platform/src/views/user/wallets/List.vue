...
const table = reactive({
	...
	{
		dataKey: 'user_name',
		title: t('user.wallets.name.user_id'),
		key: 'user_id',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'balance',
		title: t('user.wallets.name.balance'),
		key: 'balance',
		align: 'center',
		width: 150,
	},
	{
		...
		title: t('user.wallets.name.reward_points'),
		key: 'reward_points',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'currency',
		title: t('user.wallets.name.currency'),
		key: 'currency',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'isStop',
		title: t('user.wallets.name.isStop'),
		key: 'isStop',
		align: 'center',
		width: 100,
		cellRenderer: (props: any): any => {
...
const handleEditCopy = (id: number, type: string = 'edit') => {
	request(t('config.VITE_HTTP_API_PREFIX') + '/user/wallets/info', { id: id }).then((res) => {
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/user/wallets/list', param)
	...
	<ElRow class="main-table-tool">
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
				<MyExportButton :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/user/wallets/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
