...
<template>
	<ElForm class="query-form" :ref="(el: any) => { queryForm.ref = el }" :model="queryCommon.data" :inline="true" @keyup.enter="queryForm.submit">
		...
		<ElFormItem prop="user_id">
			<MySelect v-model="queryCommon.data.user_id" :placeholder="t('user.wallets.name.user_id')" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/user/user/list' }" />
		</ElFormItem>
<!--		<ElFormItem prop="currency">-->
<!--			<ElInput v-model="queryCommon.data.currency" :placeholder="t('user.wallets.name.currency')" minlength="1" maxlength="10" :clearable="true" />-->
<!--		</ElFormItem>-->
		<ElFormItem prop="isStop" style="width: 120px;">
			<ElSelectV2 v-model="queryCommon.data.isStop" :options="tm('common.status.whether')" :placeholder="t('user.wallets.name.isStop')" :clearable="true" />
		</ElFormItem>
