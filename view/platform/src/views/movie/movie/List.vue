<script setup lang="ts">
  ...
  const table = reactive({
    ...
    {
      dataKey: 'movie_id',
      title: t('movie.movie.name.movie_id'),
      key: 'movie_id',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'movie_name',
      title: t('movie.movie.name.movie_name'),
      key: 'movie_name',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'movie_real_name',
      title: t('movie.movie.name.movie_real_name'),
      key: 'movie_real_name',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'movie_picture',
      title: t('movie.movie.name.movie_picture'),
      key: 'movie_picture',
      align: 'center',
      width: 150,
      cellRenderer: (props: any): any => {
        if (!props.rowData.movie_picture) {
          return
        }
        //const imageList= JSON.parse(props.rowData.avatar)
        const imageList = [props.rowData.movie_picture]
        const row = props.rowData

        if (row.movie_picture && !row.imageLoaded) {
          row.imageSrc = row.movie_picture;
        ...
        //           'style': 'width: 100px; height:100px;',    //不想显示滚动条，需设置table属性row-height增加行高
        //           'src': item,
        //           'lazy': true,
    ...
    {
      dataKey: 'plot_photo',
      title: t('movie.movie.name.plot_photo'),
      key: 'plot_photo',
      align: 'center',
      width: 150,
      cellRenderer: (props: any): any => {

        if (!props.rowData.plot_photo) {
          return
        }
        //const imageList= JSON.parse(props.rowData.avatar)
        const imageList = [props.rowData.plot_photo]
        const row = props.rowData

        if (row.plot_photo && !row.imageLoaded1) {
          row.imageSrc1 = row.plot_photo;
          row.imageLoaded1 = true;
        }
    ...
    {
      dataKey: 'translated_name',
      title: t('movie.movie.name.translated_name'),
      key: 'translated_name',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'sub_intros',
      title: t('movie.movie.name.sub_intros'),
      key: 'sub_intros',
      align: 'center',
      width: 200,
          hidden: true
    ...
    {
      dataKey: 'years',
      title: t('movie.movie.name.years'),
      key: 'years',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'place',
      title: t('movie.movie.name.place'),
      key: 'place',
      align: 'center',
      width: 150,
    },
    {
      ...
      title: t('movie.movie.name.category_name'),
      key: 'category_name',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'category_ids',
      title: t('movie.movie.name.category_ids'),
      key: 'category_ids',
      align: 'center',
      width: 150,
    },
    {
      ...
      title: t('movie.movie.name.language_name'),
      key: 'language_name',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'subtitle',
      title: t('movie.movie.name.subtitle'),
      key: 'subtitle',
      align: 'center',
      width: 150,
    },
    {
      ...
      title: t('movie.movie.name.release_date'),
      key: 'release_date',
      align: 'center',
      width: 150,
    },
    ...
    {
      dataKey: 'doubandb',
      title: t('movie.movie.name.doubandb'),
      key: 'doubandb',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'score',
      title: t('movie.movie.name.score'),
      key: 'score',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'awards',
      title: t('movie.movie.name.awards'),
      key: 'awards',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'times',
      title: t('movie.movie.name.times'),
      key: 'times',
      align: 'center',
      width: 150,
    },
    ...
    {
      dataKey: 'director',
      title: t('movie.movie.name.director'),
      key: 'director',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'scriptwriter',
      title: t('movie.movie.name.scriptwriter'),
      key: 'scriptwriter',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'actors',
      title: t('movie.movie.name.actors'),
      key: 'actors',
      align: 'center',
      width: 200,
          hidden: true
    ...
    {
      dataKey: 'intros',
      title: t('movie.movie.name.intros'),
      key: 'intros',
      align: 'center',
      width: 200,
          hidden: true
    ...
    {
      dataKey: 'down_urls',
      title: t('movie.movie.name.down_urls'),
      key: 'down_urls',
      align: 'center',
      width: 200,
          hidden: true
    ...
    {
      dataKey: 'video_size',
      title: t('movie.movie.name.video_size'),
      key: 'video_size',
      align: 'center',
      width: 150,
    },
    {
      dataKey: 'video_resolution',
      title: t('movie.movie.name.video_resolution'),
    ...
    {
      dataKey: 'isUpdate',
      title: t('movie.movie.name.isUpdate'),
      key: 'isUpdate',
      align: 'center',
      width: 100,
      cellRenderer: (props: any): any => {
        return [
          h(ElSwitch as any, {
            'model-value': props.rowData.isUpdate,
            'active-value': 1,
            'inactive-value': 0,
            'inline-prompt': true,
    ...
    {
      dataKey: 'isStop',
      title: t('movie.movie.name.isStop'),
      key: 'isStop',
      align: 'center',
      width: 100,
      cellRenderer: (props: any): any => {
  ...
  const handleEditCopy = (id: number, type: string = 'edit') => {
    request(t('config.VITE_HTTP_API_PREFIX') + '/movie/movie/info', { id: id }).then((res) => {
  ...
  const handleDelete = (idArr: number[]) => {
    ElMessageBox.confirm('', {
      ...
      request(t('config.VITE_HTTP_API_PREFIX') + '/movie/movie/del', { idArr: idArr }, true).then((res) => {
        getList()
      }).catch(() => { })
    }).catch(() => { })
  }
  ...
  const getList = async (resetPage: boolean = false) => {
    ...
    table.loading = true
    try {
      const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/movie/movie/list', param)
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
                <MyExportButton :headerList="table.columns"
                    :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/movie/movie/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
		...
		<ElAutoResizer>
			<template #default="{ height, width }">
				<ElTableV2 class="main-table" :columns="table.columns" :data="table.data" :sort-by="table.sort"
					@column-sort="table.handleSort" :width="width" :height="height" :fixed="true" :row-height="100">
