...
<template>
	<ElForm class="query-form" :ref="(el: any) => { queryForm.ref = el }" :model="queryCommon.data" :inline="true" @keyup.enter="queryForm.submit">
		...
		<ElFormItem prop="isActive" style="width: 120px;">
			<ElSelectV2 v-model="queryCommon.data.isActive" :options="tm('common.status.whether')" :placeholder="t('app.cardCategories.name.isActive')" :clearable="true" />
		</ElFormItem>
		<ElFormItem prop="isStop" style="width: 120px;">
			<ElSelectV2 v-model="queryCommon.data.isStop" :options="tm('common.status.whether')" :placeholder="t('app.cardCategories.name.isStop')" :clearable="true" />
		</ElFormItem>
