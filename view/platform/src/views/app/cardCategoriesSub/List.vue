...
const table = reactive({
	...
	{
		dataKey: 'cate_id',
		title: t('app.cardCategoriesSub.name.cate_id'),
		key: 'cate_id',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'name',
		title: t('app.cardCategoriesSub.name.name'),
		key: 'name',
		align: 'center',
		width: 350,
	},
	{
		dataKey: 'rate',
		title: t('app.cardCategoriesSub.name.rate'),
		key: 'rate',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'minAcceptableAmount',
		title: t('app.cardCategoriesSub.name.minAcceptableAmount'),
		key: 'minAcceptableAmount',
		align: 'center',
		width: 150,
	},
	{
		dataKey: 'announcements',
		title: t('app.cardCategoriesSub.name.announcements'),
		key: 'announcements',
		align: 'center',
		width: 200,
		hidden: true,
	...
	{
		dataKey: 'sort',
		title: t('app.cardCategoriesSub.name.sort'),
		key: 'sort',
		align: 'center',
		width: 100,
		sortable: true,
			...
			if (props.rowData.editSort) {
				...
				return [
					h(ElInputNumber as any, {
						...
						'placeholder': t('app.cardCategoriesSub.tip.sort'),
						'precision': 0,
						'min': 0,
						'max': 100,
						'step': 1,
	...
	{
		dataKey: 'isActive',
		title: t('app.cardCategoriesSub.name.isActive'),
		key: 'isActive',
		align: 'center',
		width: 100,
		cellRenderer: (props: any): any => {
	...
	{
		dataKey: 'isStop',
		title: t('app.cardCategoriesSub.name.isStop'),
		key: 'isStop',
		align: 'center',
		width: 100,
		cellRenderer: (props: any): any => {
	...
	{
		dataKey: 'sub_id',
		title: t('app.cardCategoriesSub.name.sub_id'),
		key: 'sub_id',
		align: 'center',
		width: 150,
	},
...
const handleEditCopy = (id: number, type: string = 'edit') => {
	request(t('config.VITE_HTTP_API_PREFIX') + '/app/cardCategoriesSub/info', { id: id }).then((res) => {
...
const handleDelete = (idArr: number[]) => {
	ElMessageBox.confirm('', {
		...
		request(t('config.VITE_HTTP_API_PREFIX') + '/app/cardCategoriesSub/del', { idArr: idArr }, true).then((res) => {
			getList()
		}).catch(() => { })
	}).catch(() => { })
}
...
const getList = async (resetPage: boolean = false) => {
	...
	table.loading = true
	try {
		const res = await request(t('config.VITE_HTTP_API_PREFIX') + '/app/cardCategoriesSub/list', param)
	...
	<ElRow class="main-table-tool">
		...
		<ElCol :span="8" style="text-align: right;">
			<ElSpace :size="10" style="height: 100%;">
				<MyExportButton :headerList="table.columns" :api="{ code: t('config.VITE_HTTP_API_PREFIX') + '/app/cardCategoriesSub/list', param: { filter: queryCommon.data, sort: table.sort.key + ' ' + table.sort.order } }" />
