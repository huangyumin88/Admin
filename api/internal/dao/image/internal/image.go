...
// ImageDao is the data access object for table ai_image.
...
// ImageColumns defines and stores column names for table ai_image.
...
// imageColumns holds the columns for table ai_image.
...
func NewImageDao() *ImageDao {
	return &ImageDao{
		...
		primaryKey: func() string {
			return reflect.ValueOf(imageColumns).Field(0).String()
		}(),
		columnArr: func() []string {
			v := reflect.ValueOf(imageColumns)
			count := v.NumField()
...
// 主键ID
func (dao *ImageDao) PrimaryKey() string {
	return dao.primaryKey
}

// 所有字段的数组
func (dao *ImageDao) ColumnArr() []string {
	return dao.columnArr
}

// 所有字段的数组（该格式更方便使用）
func (dao *ImageDao) ColumnArrG() *garray.StrArray {
