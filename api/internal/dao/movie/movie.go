...
import (
	"api/internal/dao/movie/internal"
	"context"
	"database/sql"
	"strings"
...
// internalMovieDao is internal type for wrapping internal DAO implements.
type internalMovieDao = *internal.MovieDao
...
var (
	// Movie is globally public accessible object for table web_movie operations.
	Movie = movieDao{
		internal.NewMovieDao(),
	}
)

// 解析分库
func (daoThis *movieDao) ParseDbGroup(ctx context.Context, dbGroupSeldata ...map[string]interface{}) string {
	group := daoThis.Group()
	// 分库逻辑
...
// 解析分表
func (daoThis *movieDao) ParseDbTable(ctx context.Context, dbTableSelData ...map[string]interface{}) string {
	table := daoThis.Table()
	// 分表逻辑
...
// 解析分库分表（对外暴露使用）
func (daoThis *movieDao) ParseDbCtx(ctx context.Context, dbSelDataList ...map[string]interface{}) *gdb.Model {
...
// 解析insert
func (daoThis *movieDao) ParseInsert(insert map[string]interface{}, fill ...bool) gdb.ModelHandler {
...
// hook insert
func (daoThis *movieDao) HookInsert(data map[string]interface{}) gdb.HookHandler {
...
// 解析update
func (daoThis *movieDao) ParseUpdate(update map[string]interface{}, fill ...bool) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
...
// hook update
func (daoThis *movieDao) HookUpdate(data map[string]interface{}, idArr ...int) gdb.HookHandler {
	return gdb.HookHandler{
...
// hook delete
func (daoThis *movieDao) HookDelete(idArr ...int) gdb.HookHandler {
	return gdb.HookHandler{
...
// 解析field
func (daoThis *movieDao) ParseField(field []string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		afterField := []string{}
		for _, v := range field {
			switch v {
			...
			case `label`:
				m = m.Fields(daoThis.Table() + `.` + daoThis.Columns().MovieName + ` AS ` + v)
			default:
...
// hook select
func (daoThis *movieDao) HookSelect(afterField []string) gdb.HookHandler {
	return gdb.HookHandler{
...
// 解析filter
func (daoThis *movieDao) ParseFilter(filter map[string]interface{}, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for k, v := range filter {
			...
			case `label`:
				m = m.WhereLike(daoThis.Table()+`.`+daoThis.Columns().MovieName, `%`+gconv.String(v)+`%`)
			case `timeRangeStart`:
...
// 解析group
func (daoThis *movieDao) ParseGroup(group []string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range group {
			switch v {
			case `id`:
...
// 解析order
func (daoThis *movieDao) ParseOrder(order []string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		for _, v := range order {
...
// 解析join
func (daoThis *movieDao) ParseJoin(joinCode string, joinTableArr *[]string) gdb.ModelHandler {
	return func(m *gdb.Model) *gdb.Model {
		switch joinCode {
		/* case Xxxx.Table():
		relTable := Xxxx.Table()
