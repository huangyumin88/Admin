...
type CardCategoriesSubListReq struct {
	g.Meta `path:"/cardCategoriesSub/list" method:"post" tags:"平台后台/子分类" sm:"列表"`
...
/*--------详情 开始--------*/
type CardCategoriesSubInfoReq struct {
	g.Meta `path:"/cardCategoriesSub/info" method:"post" tags:"平台后台/子分类" sm:"详情"`
	Id     uint     `json:"id" v:"required|min:1" dc:"ID"`
	Field  []string `json:"field" v:"distinct|foreach|min-length:1" dc:"查询字段，传值参考返回的字段名，默认返回全部字段。注意：如前端页面所需字段较少，建议传指定字段，可大幅减轻服务器及数据库压力"`
}

type CardCategoriesSubInfoRes struct {
	Info CardCategoriesSubInfo `json:"info" dc:"详情"`
}

type CardCategoriesSubInfo struct {
	Id                  *uint       `json:"id,omitempty" dc:"ID"`
	Label               *string     `json:"label,omitempty" dc:"标签。常用于前端组件"`
	CateId              *int        `json:"cate_id,omitempty" dc:"主分类ID"`
	Name                *string     `json:"name,omitempty" dc:"card 名字"`
...
/*--------新增 开始--------*/
type CardCategoriesSubCreateReq struct {
	g.Meta              `path:"/cardCategoriesSub/create" method:"post" tags:"平台后台/子分类" sm:"创建"`
	CateId              *int    `json:"cate_id,omitempty" v:"min:1" dc:"主分类ID"`
	Name                *string `json:"name,omitempty" v:"required|max-length:255|regex:^[\\p{L}\\p{M}\\p{N}_-]+$" dc:"card 名字"`
	Rate                *int    `json:"rate,omitempty" v:"" dc:"费率"`
	MinAcceptableAmount *int    `json:"minAcceptableAmount,omitempty" v:"" dc:"最低可接受金额"`
	Announcements       *string `json:"announcements,omitempty" v:"" dc:"注意事项"`
...
/*--------修改 开始--------*/
type CardCategoriesSubUpdateReq struct {
	g.Meta              `path:"/cardCategoriesSub/update" method:"post" tags:"平台后台/子分类" sm:"更新"`
...
/*--------删除 开始--------*/
type CardCategoriesSubDeleteReq struct {
	g.Meta `path:"/cardCategoriesSub/del" method:"post" tags:"平台后台/子分类" sm:"删除"`
	IdArr  []uint `json:"idArr,omitempty" v:"required|distinct|foreach|min:1" dc:"ID数组"`
}
