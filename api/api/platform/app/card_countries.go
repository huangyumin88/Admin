...
type CardCountriesListReq struct {
	g.Meta `path:"/cardCountries/list" method:"post" tags:"平台后台/国家" sm:"列表"`
...
/*--------详情 开始--------*/
type CardCountriesInfoReq struct {
	g.Meta `path:"/cardCountries/info" method:"post" tags:"平台后台/国家" sm:"详情"`
	Id     uint     `json:"id" v:"required|min:1" dc:"ID"`
	Field  []string `json:"field" v:"distinct|foreach|min-length:1" dc:"查询字段，传值参考返回的字段名，默认返回全部字段。注意：如前端页面所需字段较少，建议传指定字段，可大幅减轻服务器及数据库压力"`
}

type CardCountriesInfoRes struct {
	Info CardCountriesInfo `json:"info" dc:"详情"`
}
...
/*--------删除 开始--------*/
type CardCountriesDeleteReq struct {
	g.Meta `path:"/cardCountries/del" method:"post" tags:"平台后台/国家" sm:"删除"`
	IdArr  []uint `json:"idArr,omitempty" v:"required|distinct|foreach|min:1" dc:"ID数组"`
}
